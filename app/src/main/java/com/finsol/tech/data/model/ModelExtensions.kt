...
fun Array<OrderHistoryModel>.maskOrderHistoryModel(): Array<OrderHistoryModel> {
    this.map {
        it.LTP = if (it.LTP.isNullOrBlank()) {
            "-"
        } else {
            it.LTP
        }
...
fun PendingOrderModel.toNonNullModel(): PendingOrderModel {
    ...
    val PriceSend: Int = if (this.PriceSend == null) 0 else this.PriceSend
    ...
    val StopPrice: Int = if (this.StopPrice == null) 0 else this.StopPrice
